...
BEGIN
  ...
  SELECT * INTO v_host FROM public.profiles WHERE id = v_request.host_id;
  SELECT * INTO v_user FROM public.profiles WHERE id = v_request.user_id;
...
BEGIN
  ...
  SELECT
    ...
    (SELECT wallet_balance::INTEGER FROM public.profiles WHERE id = p_host_id),
