...
CREATE TABLE user_reports (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  reporter_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL, -- Can be null for system-generated
...
BEGIN
  ...
  -- Get host details
  SELECT * INTO v_host FROM public.profiles WHERE id = v_payout.host_id;
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM public.profiles WHERE last_login_at >= CURRENT_DATE),
    (SELECT COUNT(*) FROM public.profiles WHERE role = 'host'),
...
BEGIN
  INSERT INTO platform_statistics (
    ...
    v_stat_date,
    (SELECT COUNT(*) FROM public.profiles WHERE role != 'admin'),
    (SELECT COUNT(*) FROM public.profiles WHERE role != 'admin' AND created_at >= v_stat_date),
    (SELECT COUNT(*) FROM public.profiles WHERE role = 'host'),
    (SELECT COUNT(*) FROM public.profiles WHERE role = 'host' AND created_at >= v_stat_date),
    (SELECT COUNT(*) FROM hotspots),
    (SELECT COUNT(*) FROM hotspots WHERE is_online = TRUE),
